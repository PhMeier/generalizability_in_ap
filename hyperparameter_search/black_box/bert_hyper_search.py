...
def hp_space_ray(trial):
    ...
    return {
        ...
        "num_train_epochs": tune.choice([3,5,8,10])
    }
...
if __name__ == "__main__":
    ...
    assert "bert" in training_file
    assert "bert" in val_file
    N_TRIALS = args.n_trials #60
    OUTPUT_DIR = f"./bert/{output_directory}/"
    LOG_DIR = f"./bert/{output_directory}/logs"
    ...
    #tokenizer.save_pretrained(OUTPUT_DIR + "tokenizer/")
    #final_trainer.save_model(OUTPUT_DIR + "best_model/")
    #with open(OUTPUT_DIR+"best_hyperparameters.json", "w") as f:
    #    json.dump(best_run.hyperparameters, f, indent=4)
