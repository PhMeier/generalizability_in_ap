...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = config["output_path"] + f"/roberta_{SEED}_{mask_type}/"
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    LOG_DIR = f"../pan_14/roberta_{SEED}_{mask_type}/logs"
    assert "roberta" == config["model_name_or_path"] # check if everything is correct

    tokenizer = AutoTokenizer.from_pretrained(config["tokenizer_name_or_path"], max_len=512)
