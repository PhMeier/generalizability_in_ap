...
if __name__ == "__main__":
    ...
    assert "xlnet" in config["hyperparameter_file"]# check if everything is correct

    with open(config["hyperparameter_file"], "r") as f:
        best_params = json.load(f)

    OUTPUT_DIR = config["output_path"] + f"/xlnet_{SEED}/"
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    LOG_DIR = f"../pan_14/xlnet_{SEED}/logs"

    assert "xlnet" == config["model_name_or_path"] # check if everything is correct


    tokenizer = XLNetTokenizer.from_pretrained(config["tokenizer_name_or_path"], max_length=512)
    model = XLNetForSequenceClassification.from_pretrained(config["model_type"])
    ...
    assert "xlnet" in config["train_file"]
    assert "xlnet" in config["val_file"]
    ...
    dataset_train = train_dataset.map(lambda e: tokenizer(e["document"], truncation=True, padding="max_length", max_length=512), batched=True)
    dataset_train = dataset_train.shuffle(seed=42)
    dataset_val = val_dataset.map(lambda e: tokenizer(e["document"], truncation=True, padding="max_length", max_length=512), batched=True)
