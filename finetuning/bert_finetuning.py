...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = config["output_path"] + f"/bert_{SEED}/"
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    LOG_DIR = f"../pan_14/bert_{SEED}/logs"
    assert config["model_name_or_path"] == MODEL_NAME # check if everything is correct

    tokenizer = BertTokenizer.from_pretrained(config["tokenizer_name_or_path"], max_length=512)
