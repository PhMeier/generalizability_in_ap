...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = config["output_path"] + f"/xlnet_{SEED}_{mask_type}/"
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    LOG_DIR = f"../pan_14/xlnet_{SEED}_{mask_type}/logs"
    ...
    assert "xl" in config["train_file"]
    assert "xl" in config["val_file"]
