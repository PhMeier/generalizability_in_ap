...
def parse_email(pathname, orig=True):
    if path.isdir(pathname):
        ...
        for child in listdir(pathname):
            ...
            if child[0] != ".":
                parse_email(path.join(pathname, child), True)#False)
    else:
        ...
        with open(pathname) as TextFile:
            try:
                ...
                print("Unicode-Decode-Error: ", pathname)
                text = ""
            try:
                ...
                print("Failed to parse %s" % pathname)
                return None
    ...
    if orig:
        try:
            with open(f'./messages/{OUTPUT}_messages.json', 'w') as f:
                json.dump(feeds, f)
            with open(f'./users/{OUTPUT}_users.json', 'w') as f:
                json.dump(users, f)
            with open(f'./threads/{OUTPUT}_threads.json', 'w') as f:
                json.dump(threads, f)
            """
...
#filename = sys.argv[1]
#OUTPUT = filename.split("/")[-2].strip()
#parse_email(filename)

if __name__ == "__main__":
    with open("parse_missed_persons.txt", "r", encoding="utf-8") as f: # "all_sent_paths_cleaned.txt"
        for line in f:
            line = line.strip()
            filename = line
            OUTPUT = filename.split("/")[-2].strip()
